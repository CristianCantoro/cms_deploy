...
def build_text_list(base_dir, task_type):
    text_md = os.path.join(TEXT_DIRNAME, TEXT_MD)
    text_pdf = os.path.join(TEXT_DIRNAME, TEXT_PDF)

    def make_pdf(assume=None):
        call(os.path.join(base_dir,TEXT_DIRNAME),
             ['pandoc', '--template=/data/template.tex', TEXT_MD,
              '-o', TEXT_PDF])
    actions = []
    if os.path.exists(text_md):
        actions.append(([text_md], [text_pdf],
